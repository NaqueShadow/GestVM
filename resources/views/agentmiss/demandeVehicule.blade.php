...
@section('content')
    ...
    <div style="margin: 2%; overflow-y: auto; overflow-x: hidden">
        <form class="mt-3" method="post" action="{{route('mission.storeDemande')}}" id="form">
            @csrf
            <div class="align-items-center">
                <fieldset>
                    <div class="form-row" style="padding: 2%; margin-bottom: auto; border: 1px solid mediumseagreen; border-radius: 15px;">
                        <legend class="mb-3 text-black-50">Nouvelle demande de véhicule</legend>
                        ...
                        <div  class="form-group col-md-5 col-sm-6">
                            <div class="">
                                <div class="font-weight-bold">Activité comptable</div>
                                <select name="idActivite" id="idActivite" class="selectpicker form-control @error('idActivite') is-invalid @enderror" @include('include.selectOption')>
                                    ...
                                    @foreach($activites as $activite)
                                        <option value="{{$activite->code}}" {{ old('idActivite') == $activite->code ? 'selected' : ''}}>{{ $activite->designation }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @error('idActivite')
                        ...
                        <div  class="form-group col-6 offset-sm-0 offset-md-1">
                            <div class="">
                                <div class="font-weight-bold">Entité</div>
                                <select name="idEntite" id="idEntite" class="selectpicker form-control @error('idEntite') is-invalid @enderror" @include('include.selectOption')>
                                    ...
                                    @foreach($entites as $entite)
                                        <option value="{{$entite->id}}" {{ old('idEntite') == $entite->id ? 'selected' : ''}}>{{$entite->designation}}</option>
                                    @endforeach
                                </select>
                            </div>
                            @error('idEntite')
                        ...
                        <div  class="form-group col-4">
                            <div class="input-group">
                                ...
                                <select name="typeV" id="typeV" class="selectpicker border form-control @error('typeV') is-invalid @enderror">
                                    <option value="" class="text-light">...</option>
                        ...
                        <div  class="form-group col-3">
                            <div class="input-group">
                                ...
                                <input type="codeV" name="codeV" id="codeV" placeholder="..." value="{{ old('codeV') }}" class="form-control @error('codeV') is-invalid @enderror">
                            </div>
                            @error('codeV')
                            <div class="invalide-feedBack() text-danger">
                                véhicule inexistant
                            ...
                            <div class="input-group">
                                ...
                                <select name="idChauf" id="idChauf" class="selectpicker form-control @error('idChauf') is-invalid @enderror" @include('include.selectOption')>
                                    ...
                                    @foreach($chauffeurs as $chauffeur)
                                        <option value="{{$chauffeur->matricule}}" {{ old('idChauf') == $chauffeur->matricule ? 'selected' : ''}}>{{$chauffeur->nom}} {{$chauffeur->prenom}} ({{$chauffeur->matricule}})</option>
                                    @endforeach
                                </select>
                            </div>
                            @error('idChauf')
