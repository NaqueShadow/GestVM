...
@section('content')
    ...
    <div class="card mt-2 h-100 align-content-center text-dark" style="margin: auto; box-shadow: 1px 1px 2px mediumseagreen; border-radius: 15px; width: 100%;">
        ...
        <div class="card-body h-100" style="color: #284563; overflow: auto;">
            <div class="">
                <form class="" method="post" action="{{route('entite.filtre')}}" id="form">
                    @method('PATCH')
                    @csrf
                    <div  class="form-group form-row mb-0">
                        ...
                        <select name="direction" id="direction" required class="selectpicker col-4 form-control" @include('include.selectOption')>
                            <option value=""></option>
                            @foreach($directions as $direction)
                                <option value="{{ $direction->id }}" {{ $direction->id==$idDr?'selected':'' }}>{{ $direction->designation }}</option>
                            @endforeach
                        </select>
                ...
                <table class="table table-striped table-hover">
                    ...
                    @foreach($entites as $entite)
                        <tr>
                            ...
                            <td>{{ $entite->abbreviation }}</td>
                            <td>{{ $entite->idDirection ? $entite->direction->designation:'' }}</td>
                            <td>
                                <div class="dropdown">
                                    <button title="exporter" class="btn btn-link text-primary pt-0 pb-0 dropdown-toggle" data-toggle="dropdown" >
                                    </button>
                                    <div class="dropdown-menu dropdown-menu-sm-right">
                                        <a href="{{ route('entite.edit', ['entite' => $entite->id]) }}" class="dropdown-item">
                                        ...
                                        <form method="post" class="dropdown-item" action="{{ route('entite.destroy', ['entite' => $entite->id]) }}" style="display: inline;">
                                            @csrf
                                            @method ('DELETE')
            ...
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Enregistrement d'une entité ou service</h5>
                    ...
                    <div class="modal-body">
                        <form class="mt-5" method="post" action="{{route('entite.store')}}" id="form">
                            @csrf
                            ...
                            <div class="form-group input-group col-12 row">
                                <label class="col-3">Direction</label>
                                <div class="col-9">
                                    <select name="idDirection" id="idDirection" required class="form-control @error('idDirection') is-invalid @enderror">
                                        <option value=""></option>
                                        @foreach($directions as $direction)
                                            <option value="{{ $direction->id }}" {{$direction->id==old('idDirection')?'selected':''}}>{{ $direction->designation }}</option>
                                        @endforeach
                                    </select>
                                    @error('idDirection')
                                    <div class="invalide-feedBack() text-danger">
